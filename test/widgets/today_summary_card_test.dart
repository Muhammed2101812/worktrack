...
class MockEntriesNotifier extends EntriesNotifier {
  final Future<List<WorkEntry>> Function() _buildFn;
  MockEntriesNotifier(this._buildFn);
  ...
  group('TodaySummaryCard Widget Tests', () {
    testWidgets('should display loading state', (WidgetTester tester) async {
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            entriesProvider.overrideWith(() => MockEntriesNotifier(() => Completer<List<WorkEntry>>().future)),
          ],
    ...
    testWidgets('should display summary with entries',
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            entriesProvider.overrideWith(() => MockEntriesNotifier(() => Future.value(entries))),
          ],
          child: const MaterialApp(
            home: Scaffold(
              body: TodaySummaryCard(),
    ...
    testWidgets('should display 0 hours when no entries',
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            entriesProvider.overrideWith(() => MockEntriesNotifier(() => Future.value([]))),
          ],
          child: const MaterialApp(
            home: Scaffold(
              body: TodaySummaryCard(),
